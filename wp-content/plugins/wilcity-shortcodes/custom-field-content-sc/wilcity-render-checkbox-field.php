...
function wilcityRenderCheckboxField($aAtts){
	...
	$aRawValues = is_array($aRawValues) ? $aRawValues : array($aRawValues);
	$aValues = array();
	...
	foreach ($aOptions as $key => $val){
		...
		if ( (empty($aRawValues) || !in_array($key, $aRawValues)) && $aAtts['print_unchecked'] == 'no' ){
			continue;
		}
		...
		if ( !empty($aRawValues) && in_array($key, $aRawValues) ){
			$aItem['unChecked'] = 'no';
		}else{
			$aItem['unChecked'] = 'yes';
		}
