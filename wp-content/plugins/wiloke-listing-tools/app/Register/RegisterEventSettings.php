...
class RegisterEventSettings {
	...
	public function setDefaultSearchFields(){
		...
		$aUsedFields = array_filter($aDefaults, function($aField){
			return isset($aField['isDefault']);
		});
		SetSettings::setOptions(General::getSearchFieldsKey(self::$forPostType), $aUsedFields);
	}
...
//		unset($this->aAllSections['price_range']);
...
//		SetSettings::setOptions(General::getUsedSectionKey($_POST['postType']), $aValues);
	...
	private function setFieldDefaults($isFocus=false){
		if ( !$isFocus ){
			...
			$aUsedSections = GetSettings::getOptions(wilokeListingToolsRepository()->get('event-settings:designFields', true)->sub('usedSectionKey'));
...
//		$aTextFields = $this->aAllSections['text']['fields'][0];
//		$aRawDefaultFields['video'] = $this->aAllSections['text'];
//		unset($aRawDefaultFields['video']['fields'][0]);
//		$aRawDefaultFields['video']['fields']['settings'] = $aTextFields;
//		$aRawDefaultFields['video']['key'] = 'video';
...
//		$aListingAddress  = $this->aAllSections['listing_address']['fields'][0];
//		$aRegion  = $this->aAllSections['listing_address']['fields'][1];
//		$aRawDefaultFields['listing_address'] = $this->aAllSections['listing_address'];
//		unset($aRawDefaultFields['listing_address']['fields'][0]);
//		unset($aRawDefaultFields['listing_address']['fields'][1]);
//		$aRawDefaultFields['listing_address']['fields']['address'] = $aListingAddress;
//		$aRawDefaultFields['listing_address']['fields']['listing_location'] = $aRegion;

		$aDefaultFields[] = $aRawDefaultFields['listing_title'];
		$aDefaultFields[] = $aRawDefaultFields['featured_image'];
//		$aDefaultFields[] = $aRawDefaultFields['video'];
		$aDefaultFields[] = $aRawDefaultFields['event_calendar'];
//		$aDefaultFields[] = $aRawDefaultFields['listing_address'];

//		unset($aRawDefaultFields['listing_address']);
		unset($aRawDefaultFields['listing_title']);
//		unset($aRawDefaultFields['video']);
