...
//		add_action('admin_init', array($this, 'setDefaultAddListing'));
	...
	public function setDefaultAddListingSettings(){
		...
		if ( !$this->isResetDefault ){
			if ( GetSettings::getOptions(wilokeListingToolsRepository()->get('addlisting:usedSectionKey')) ){
				return false;
			}
		}
		$this->getUsedSections();
		SetSettings::setOptions(wilokeListingToolsRepository()->get('addlisting:usedSectionKey'), $this->aUsedSections);
	}
	...
	protected function getSingleNavigation(){
		...
		$this->aSingleNav = !empty($aNavSettings) ? $aNavSettings + $aDefault : $aDefault;
	}
