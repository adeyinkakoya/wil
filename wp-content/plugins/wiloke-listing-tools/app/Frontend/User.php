...
class User {
	...
	public static $aCanSubmitListingRoles = array('contributor', 'seller');
	...
	public static function isAccountConfirmed(){
		if ( !\WilokeThemeOptions::isEnable('toggle_confirmation') ){
			return true;
		}

		return GetSettings::getUserMeta(get_current_user_id(), 'confirmed');
	}

	public static function canSubmitListing($userID=''){
	...
	public static function countUserPosts($userID, $type, $perm=''){
		...
		$query = "SELECT post_status, COUNT( * ) AS num_posts FROM {$wpdb->posts} WHERE post_type = %s AND post_author=%d AND post_status IN('".implode("','", $aPostStatusKey)."')";
	...
	public static function countUserPostsByPostTypes($userID, $countEventOnly=false){
		$total = 0;
		if ( $countEventOnly ){
			$aPostTypeKeys = General::getPostTypeKeys(false, $countEventOnly);
	...
	public static function renderFollower($authorID){
		...
		?>
		<span id="wilcity-follower-number-<?php echo esc_attr($authorID); ?>" class="wilcity-follower author-hero_rightText__1Yfm7"><span class="wilcity-print-number color-primary"><?php echo esc_html(General::convertToNiceNumber($followers)); ?></span> <?php echo _n('Follower', 'Followers', $nFollowers, 'wiloke-listing-tools'); ?></span>
		<?php
	}
	...
	public static function renderFollowing($authorID){
		...
		?>
		<span class="wilcity-following author-hero_rightText__1Yfm7"><span class="color-primary"><?php echo esc_html(General::convertToNiceNumber($followings)); ?></span> <?php esc_html_e('Following', 'wiloke-listing-tools'); ?></span>
		<?php
	}
