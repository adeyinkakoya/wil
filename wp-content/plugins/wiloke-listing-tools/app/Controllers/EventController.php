...
class EventController extends Controller {
	...
	public function __construct() {
		...
		add_filter('wilcity/ajax/post-comment/event', array($this, 'postComment'));
...
//        add_action('init', array($this, 'moveExpiryEventToTrash'));
	...
	public static function getEventMetaData($post){
		...
		$oTerm = \WilokeHelpers::getTermByPostID($post->ID, 'listing_location', false);
		if ( $oTerm ){
			$aMetaData[] = array(
				'icon'   => '',
				'type'   => 'term',
		...
		$oTerm = \WilokeHelpers::getTermByPostID($post->ID, 'listing_cat', false);
		if ( $oTerm ){
			$aMetaData[] = array(
				'icon'   => '',
				'type'   => 'term',
				...
				switch ($aItem['type']){
					case 'map':
                        ...
                        <div class="icon-box-1_module__uyg5F event-detail-content_location__1UYZY">
                            <div class="icon-box-1_block1__bJ25J">
                                ...
                                <div class="icon-box-1_text__3R39g"><?php echo esc_html($aItem['value']['address']); ?></div>
                            </div>
                        </div>
                        <a class="event-detail-content_showMap__3psSs color-primary wilcity-toggle-map" href="#" data-toggle-button="showmap" data-toggle-effect="slide" style="user-select: none;" data-lat="<?php echo esc_attr($aItem['value']['lat']); ?>" data-zoom="<?php echo esc_attr($wiloke->aThemeOptions['single_event_map_default_zoom']); ?>" data-max-zoom="<?php echo esc_attr($wiloke->aThemeOptions['single_event_map_max_zoom']); ?>" data-min-zoom="<?php echo esc_attr($wiloke->aThemeOptions['single_event_map_minimum_zoom']); ?>" data-mapid="wilcity-event-map-<?php echo esc_attr($post->ID); ?>" data-lng="<?php echo esc_attr($aItem['value']['lng']); ?>"><?php esc_html_e('Show map', 'wiloke-listing-tools'); ?> <i class="la la-angle-down"></i></a>
                        <div class="event-detail-content_map__1LrJO mt-15" data-toggle-content="showmap">
                            <div style="height: 200px; background-color: #e7e7ed;" id="wilcity-event-map-<?php echo esc_attr($post->ID); ?>"></div>
                        </div>
						<?php
						break;
					case 'term':
					...
					case 'single_price':
                        ...
                        <div class="icon-box-1_module__uyg5F event-detail-content_location__1UYZY">
                            ...
                            <div class="icon-box-1_text__3R39g"><a href="#"><?php echo GetWilokeSubmission::renderPrice($aItem['value']); ?></a></div>
                        </div>
						<?php
						break;
					default:
	...
	public function fetchEventsJson(){
		...
		if ( isset($_POST['orderby']) ){
			...
			$aArgs['post_status'] = array('publish', 'draft', 'expired');
		}
	...
	public function postComment($aData){
		...
		$commentID = wp_insert_post(
			array(
				...
				'post_title'    => $displayName . ' Left a Comment ' . get_the_title($aData['postID']),
				'post_content'  => $aData['content'],
				'post_parent'   => $aData['postID']
			)
		);

		global $wilcityoReview, $wiloke;
		$wiloke->aThemeOptions = \Wiloke::getThemeOptions();
		$wiloke->aConfigs['translation'] = wilcityGetConfig('translation');
		$wilcityoReview = get_post($commentID);

		do_action('wilcity/event/after-inserted-comment', $commentID, $userID, $aData['postID']);
	...
	public function detectPostStatus(){
		$aUserPlan = UserModel::getSpecifyUserPlanType('event');
	...
	private function _handlePreview() {
		...
		$aFeaturedImage  = $this->uploadFeaturedImg($this->aRawFeaturedImage);

		if ( !empty($aFeaturedImage) ){
			set_post_thumbnail($this->listingID, $aFeaturedImage['id']);
		}
	...
	public static function renderEventCalendar($post, $isReturn=false){
		...
		switch ($frequency){
			case 'occurs_once':
				...
				$aDetails[0]['time']    = esc_html(Time::toTimeFormat($aEventData['startsOn']));

				$aDetails[1]['heading'] = esc_html__('Closed at', 'wiloke-listing-tools');
				$aDetails[1]['time']    = esc_html(Time::toTimeFormat($aEventData['endsOn']));
			...
			case 'daily':
				...
				$aDetails[0]['heading'] = esc_html__('Opening at', 'wiloke-listing-tools');
				$aDetails[0]['time']    = Time::toTimeFormat($aEventData['startsOn']);

				$aDetails[1]['heading'] = esc_html__('Closed at', 'wiloke-listing-tools');
				$aDetails[1]['time']    = Time::toTimeFormat($aEventData['endsOn']);
				break;
