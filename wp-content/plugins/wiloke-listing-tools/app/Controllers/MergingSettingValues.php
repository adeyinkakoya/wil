		...
		foreach ($this->aSections as $sectionKey => $aSection){
			if ( isset($aSection['isCustomSection']) && $aSection['isCustomSection'] == 'yes' ){
				foreach ($aSection['fields'] as $fieldKey => $aField){
					...
					$this->aSections[$sectionKey]['fields'][$fieldKey]['value'] = !empty($this->listingID) ? GetSettings::getPostMeta($this->listingID, $aSection['key'], wilokeListingToolsRepository()->get('addlisting:customMetaBoxPrefix')) : '';
...
;									}
						...
						foreach ($aSection['fields'] as $fieldKey => $aField){
							...
							if ( !empty($aEventData) ){
								...
								if ( !empty($aEventData['startsOn']) && $aEventData['startsOn'] !== '0000-00-00 00:00:00' ){
									$this->aSections[ $sectionKey ]['fields'][ $fieldKey ]['value']['starts'] = date('m/d/Y', strtotime($aEventData['startsOn']));
								...
								if ( !empty($aEventData['endsOn']) && $aEventData['endsOn'] !== '0000-00-00 00:00:00' ){
									$this->aSections[ $sectionKey ]['fields'][ $fieldKey ]['value']['endsOn'] = date('m/d/Y', strtotime($aEventData['endsOn']));
					...
					default:
						...
						do_action('wiloke-listing-tools/addlisting/merging-setting-values/'.$aSection['type'], $this->aSections, $aSection, $this->listingID);
						do_action('wiloke-listing-tools/addlisting/merging-setting-values', $this->aSections, $aSection, $this->listingID);
						break;
				}
			}
		}
