...
class DokanController extends Controller {
	...
	public function __construct() {
		add_action( 'profile_update', array($this, 'toggleSelling'), 10, 1 );
		add_filter( 'dokan_get_dashboard_nav', array($this, 'dokanDashboardMenu') );
		add_action('dokan_new_product_added', array($this, 'addASignIfProductIsNotPublished'));
//		add_action('dokan_product_updated', array($this, 'addASignIfProductIsNotPublished'));
		add_action('post_updated', array($this, 'notifyToVendorProductIsPublished'), 10, 3);
		add_action('woocommerce_order_status_completed', array($this, 'notifySomeonePurchasedYourProduct'));
		add_action('dokan_after_withdraw_request', array($this, 'notifyRequestWithdrawalToAdmin'), 10, 3);
		add_action('dokan_withdraw_request_approved', array($this, 'notifyApprovedWithdrawal'), 10, 3);
		add_action('dokan_withdraw_request_cancelled', array($this, 'notifyCancelledWithdrawal'), 10, 3);

		add_action('wp_ajax_wilcity_fetch_dokan_products', array($this, 'ajaxSearchMyProducts'));
		add_filter('woocommerce_product_related_posts_query', array($this, 'filterRelateProductOnSingle'), 10, 3);

		add_action('wp_ajax_wilcity_get_my_products', array($this, 'showingDokanOnSingleListing'));
		add_action('wp_ajax_nopriv_wilcity_get_my_products', array($this, 'showingDokanOnSingleListing'));
		add_action('dokan_product_edit_after_options', array($this, 'addSendQRCodeSetting'));
		add_action('dokan_product_updated', array($this, 'saveSendQRCodeSetting'), 10, 2);
		add_action('dokan_new_product_added', array($this, 'saveSendQRCodeSetting'), 10, 2);

		add_action('admin_init', array($this, 'setDokanPagesToPageBuilderTemplate'));
	}
	...
	public function addSendQRCodeSetting($postID) {
		...
		<div class="dokan-other-options dokan-edit-row dokan-clearfix">
			<div class="dokan-section-heading" data-togglehandler="dokan_qrcode_options">
				<h2><i class="fa fa-qrcode" aria-hidden="true"></i> <?php esc_html_e('QRCode Settings', 'wiloke-listing-tools'); ?></h2>
			...
			<div class="dokan-section-content">
				<div class="dokan-form-group">
					<label class="" for="_wilcity_is_send_qrcode">
						<input name="_wilcity_is_send_qrcode" id="_wilcity_is_send_qrcode" value="yes" <?php checked($status, 'yes'); ?> type="checkbox">
						<?php esc_html_e('Sending QRCode after customer purchasing this product', 'wiloke-listing-tools'); ?> </label>
				</div>
				<div class="dokan-clearfix"></div>
				<div class="dokan-form-group">
					<label class="form-label" for="_wilcity_qrcode_description"><?php esc_html_e('Email Content', 'wiloke-listing-tools'); ?> </label>
					<textarea name="_wilcity_qrcode_description" id="_wilcity_qrcode_description" class="dokan-form-control"><?php echo esc_html($description); ?></textarea>
					<i><?php echo apply_filters('wilcity/dokan/qrcode-settings/description', esc_html_e('Eg: %h2%Check for this Event%close_h2%%breakDown%Please show us QRCode below when visiting the Event.', 'wiloke-listing-tools')); ?></i>
				</div>
			</div>
		</div>
		<?php
	...
	public function notifyApprovedWithdrawal($userID, $amount, $method){
		$withDrawID = $this->getWithDrawID(array(
			'amount' => $amount,
			'method' => $method,
			'userID' => $userID,
			'status' => 1
	...
	public function notifyCancelledWithdrawal($userID, $amount, $method){
		$withDrawID = $this->getWithDrawID(array(
			'amount' => $amount,
			'userID' => $userID,
			'method' => $method,
