...
use WilokeListingTools\Models\NotificationModel;
...
class DashboardController extends Controller {
    private static $aEndpoint = array(
	...
	public function __construct() {
		add_action('wilcity/header/after-menu', array($this, 'printProfileNavigation'), 30);
		add_action('wilcity/footer/vue-popup-wrapper', array($this, 'askForDeletingAuthorMessage'), 30);
		add_action('wp_ajax_wilcity_fetch_general_status_statistics', array($this, 'fetchGeneralStatusStatistics'));
	}
	...
	public function askForDeletingAuthorMessage(){
		...
		?>
        <wiloke-ask-for-delete-author-message-popup title="<?php esc_html_e('Do you want to delete all message from this author?', 'wiloke-listing-tools'); ?>" yes="<?php esc_html_e('Yes', 'wiloke-listing-tools'); ?>" cancel="<?php esc_html_e('Cancel', 'wiloke-listing-tools'); ?>"></wiloke-ask-for-delete-author-message-popup>
        <wiloke-ask-for-delete-single-message-popup title="<?php esc_html_e('Do you want to delete all message?', 'wiloke-listing-tools'); ?>" yes="<?php esc_html_e('Yes', 'wiloke-listing-tools'); ?>" cancel="<?php esc_html_e('Cancel', 'wiloke-listing-tools'); ?>"></wiloke-ask-for-delete-single-message-popup>
		<?php
    }
    ...
    public static function getPostStatusInfo(){
	    ...
	    foreach ($aData as $order => $aInfo){
		    $aData[$order]['total'] = self::countPostStatus($aInfo['post_status']);
	...
	public static function getNavigation($userID=null){
		...
		$aNavigation = apply_filters('wilcity/dashboard/navigation', $aNavigation);
		...
		if ( !User::canSubmitListing($userID) ){
			$aNavigation = array_filter($aNavigation, function($aItem, $key){
			   if ( in_array($key, array('favorites', 'messages', 'profile')) ){
                   return true;
               }
		...
		foreach ($aNavigation as $key => $aItem){
			...
			switch ($key){
				...
				case 'events':
					...
					break;
				case 'notifications':
					$aNavigation[$key]['count'] = absint(GetSettings::getUserMeta($userID, NotificationsModel::$countNewKey));
					break;
			}
		}
        ...
        <div class="dashboard-content_navMobile__2NsOn">
            <div class="field_module__1H6kT field_style2__2Znhe js-field">
                <div class="field_wrap__Gv92k">
                    <select id="wilcity-dashboard-nav-mobile" class="select-2 wilcity-select2-changed mt-15">
                        <?php foreach ($aNavigation as $route => $aItem): ?>
                            <option value="<?php echo esc_attr($route); ?>"><?php echo esc_html($aItem['name']); ?></option>
                        <?php endforeach; ?>
                    </select><span class="field_label__2eCP7 text-ellipsis"><?php esc_html_e('Dashboard', 'wiloke-listing-tools'); ?></span><span class="bg-color-primary"></span>
                </div>
            </div>
        </div>
        <?php
	...
	public static function printDashboardNavigation($isPrintRouter){
		...
		<ul>
                ...
                <li :class="navWrapperClass('<?php echo esc_attr($route); ?>')">
                    <?php if ( $isPrintRouter ) : ?>
                        <?php if ( isset($aItem['redirect']) ) : ?>
                            <router-link class="dashboard-nav_link__2BmK9 text-ellipsis color-primary--hover" :to="{path: '<?php echo esc_attr($route); ?>', query: {redirectTo: '<?php echo esc_attr($aItem['redirect']);?>'}}">
                        <?php else: ?>
                            <router-link class="dashboard-nav_link__2BmK9 text-ellipsis color-primary--hover" to="/<?php echo esc_attr($route); ?>">
                        <?php endif; ?>
                    <?php else: ?>
                        <?php if ( isset($aItem['redirect']) ) : ?>
                            <a class="dashboard-nav_link__2BmK9 text-ellipsis color-primary--hover" href="<?php echo esc_url($aItem['redirect']); ?>">
                        <?php else: ?>
                            <a class="dashboard-nav_link__2BmK9 text-ellipsis color-primary--hover" href="<?php echo esc_url($dashboardUrl) . '/#/'.esc_attr($route); ?>">
                        <?php endif; ?>
                    <?php endif; ?>
            ...
            <?php endforeach; ?>
			<li class="dashboard-nav_item__2798B">
				<a class="dashboard-nav_link__2BmK9 text-ellipsis color-primary--hover" href="<?php echo wp_logout_url(); ?>">
					<span class="dashboard-nav_icon__2gZV4"><i class="la la-sign-out"></i></span>
					<span class="dashboard-nav_text__x-_IZ"><?php esc_html_e('Logout', 'wiloke-listing-tools'); ?></span>
	...
	public function printProfileNavigation(){
		...
		?>
		<div id="wilcity-profile-nav-menu" style="display: none" :class="aCssClass">
			<a class="header_loginHead__3HoVP" href="#" @click.prevent="toggleProfileMenu">
				<div class="header_avatar__3lw1r bg-cover" style="background-image: url('<?php echo User::getAvatar($userID); ?>')"><img src="<?php echo User::getAvatar($userID); ?>" alt="<?php echo User::getField('display_name', $userID); ?>"/></div>
			</a>
