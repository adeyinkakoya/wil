...
//		add_filter('wiloke-listing-tools/listing/map-field-values', array(__CLASS__, 'getListingAddress'));
	...
	public function saveSettings($listingID, $post, $updated){
		...
		if ( isset($_POST['wilcity_business_hours']) && !empty($_POST['wilcity_business_hours']) ){
			...
			Listing::saveBusinessHours($listingID, $aData, $timezone);
		}
	}
	...
	public function registerMetaBoxesUseCMBTwo(){
		$this->getPostTypes();
		$aSettings = GetSettings::getOptions('promotion_plans');
		if ( empty($aSettings) ){
			return false;
		}
		...
		foreach ($aSettings as $aSetting){
			$aPositions[] = array(
				...
				'id'        => 'wilcity_promote_'.$aSetting['position'],
