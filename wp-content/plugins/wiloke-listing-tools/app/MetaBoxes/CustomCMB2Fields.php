...
//		$this->enqueueSelect2Scripts();
		...
		$aField['value'] = is_array($aField['value']) ? implode(',', $aField['value']) : $aField['value'];
		echo $field_type_object->select($aField);
...
//		$aResult = apply_filters('wiloke-listing-tools/'.General::detectPostType().'/map-field-values', $field_object_id);
