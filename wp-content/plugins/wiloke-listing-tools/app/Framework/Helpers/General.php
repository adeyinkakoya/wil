...
class General {
	...
	public static function getSecurityAuthKey(){
		...
		$auth = self::getOptionField('wilcity_security_authentication_key');
		return $auth;
	}
	...
	public static function getPostTypes($isIncludedDefaults=true, $exceptEvents=false){
		...
		if ( $isIncludedDefaults ){
			$aPostTypes =  !empty($aCustomPostTypes) ? $aCustomPostTypes + $aDefaults : $aDefaults;
		}else{
		...
		foreach ($aPostTypes as $aInfo){
			...
			$aPostTypesWithKey[$aInfo['key']]['icon'] = isset($aInfo['icon']) ? $aInfo['icon'] : '';
		}
	...
	public static function generateBusinessHours(){
		...
		$aCreatingAM = array();
		$aForm = wilokeListingToolsRepository()->get('addlisting:aFormBusinessHour');
	...
	public static function generateMetaKey($name){
		return $name . wilokeListingToolsRepository()->get('general:metaboxPrefix');
	}
	...
	public static function isRemoveWooCommerceSection() {
		return General::$isBookingFormOnSidebar;
	}
}