...
class Receipt{
	...
	public function getPrice(){
		return number_format(floatval(trim($this->aPlanSettings['regular_price'])), 2);
	}
	...
	public function setupPriceDirectly(){
		...
		if ( !isset($this->aInfo['productID']) ){
			...
			$this->total             = number_format($this->aInfo['total'], 2);
	...
	public function setupPlan(){
		...
		if ( empty($this->productID) ){
			...
			$total = $this->getPrice();
			$this->aPlan['total'] = number_format($total, 2);
			$this->subTotal     = number_format($this->aPlan['total'], 2);
			...
			if ( $this->isNonrecurringPayment ){
				if ( !empty($this->couponCode) ){
					...
					if ( !$instCoupon->isCouponExpired() && $instCoupon->isPostTypeSupported($this->aPlan['post_type']) ){
						...
						$this->aCouponInfo['amount'] = floatval($this->aCouponInfo['amount']);

						if ( $this->aCouponInfo['type'] == 'percentage' ){
							$this->aCouponInfo['discountPrice'] = number_format($this->total*$this->aCouponInfo['amount']/100, 2);
						}else{
