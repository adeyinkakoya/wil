...
class StripeRecurringPaymentMethod implements PaymentMethodInterface {
	...
	protected function charge($oCustomer){
		try{
			...
			PaymentMetaModel::set($this->paymentID, wilokeListingToolsRepository()->get('payment:paymentInfo'), $oSubscription->__toArray());
			PaymentMetaModel::set($this->paymentID, wilokeListingToolsRepository()->get('payment:stripeSubscriptionID'), $oSubscription->id);
			...
			$aResponse = apply_filters('wiloke-listing-tools/framework/payment/response', array(
				...
				'onChangedPlan'  => Session::getSession(wilokeListingToolsRepository()->get('payment:onChangedPlan'), true),
				'listingType'   => Session::getSession(wilokeListingToolsRepository()->get('payment:listingType'), true),
				'oldPlanID'     => Session::getSession(wilokeListingToolsRepository()->get('payment:oldPlanID'), true),
				'paymentID'     => $this->paymentID,
			...
			return array(
				'status' => 'success',
				'msg'    => esc_html__('Congratulations! Your payment has been processed successfully', 'wiloke-listing-tools'),
				'thankyou' => GetWilokeSubmission::getField('thankyou', true)
			);
		}catch (\Exception $oE) {
			$this->rollup();
	...
	public function createNewUser(){
		try{
			...
			return array(
				'status' => 'error',
				'msg'    => esc_html__('Your Stripe Customer ID has been deleted, please refresh the browser to re-create it.', 'wiloke-listing-tools')
			);
		}
	}
