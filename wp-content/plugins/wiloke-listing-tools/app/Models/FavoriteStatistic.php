...
class FavoriteStatistic {
	...
	public static function countMyFavorites($userID=''){
		...
		return empty($aFavorites) ? 0 : count($aFavorites);
	}
	...
	public static function countFavorites($postID){
		...
		$count = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT countLoved FROM ".self::$tableName." WHERE objectID=%d",
				$postID
			)
		);
		return abs($count);
	...
	public static function getTotalFavoritesOfAuthor($userID){
		...
		$total = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT $statisticTbl.countLoved FROM $statisticTbl LEFT JOIN $postsTbl ON ($postsTbl.ID = $statisticTbl.objectID) WHERE $postsTbl.post_status=%s AND $postsTbl.post_author=%d",
				'publish', $userID
			)
		);
		return $total ? absint($total) : 0;
