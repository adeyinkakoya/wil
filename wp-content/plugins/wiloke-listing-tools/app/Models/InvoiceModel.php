...
class InvoiceModel {
	...
	public static function getMyInvoices($limit=20, $offset=0, $userID=null){
		...
		$aResults = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT SQL_CALC_FOUND_ROWS $invoiceTbl.*, $paymentHistoryTbl.gateway FROM $invoiceTbl LEFT JOIN $paymentHistoryTbl ON ($paymentHistoryTbl.ID=$invoiceTbl.paymentID) WHERE $paymentHistoryTbl.userID=%d AND $paymentHistoryTbl.status=%s ORDER BY $invoiceTbl.created_at DESC LIMIT $offset, $limit",
				$userID, 'succeeded', $offset, $limit
			),
			ARRAY_A
		);
