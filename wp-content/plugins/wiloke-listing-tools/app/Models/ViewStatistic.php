...
class ViewStatistic {
	...
	public static function getTotalViewsOfAuthor($userID){
		...
		$total = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT $statisticTbl.countView FROM $statisticTbl LEFT JOIN $postsTbl ON ($postsTbl.ID = $statisticTbl.objectID) WHERE $postsTbl.post_status=%s AND $postsTbl.post_author=%d",
				'publish', $userID
			)
		);
	...
	public static function countViews($postID){
		...
		$count = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT countView FROM ".self::$tableName." WHERE objectID=%d",
				$postID
			)
		);
