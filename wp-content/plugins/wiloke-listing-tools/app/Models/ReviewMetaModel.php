...
class ReviewMetaModel {
	...
	public static function getReviewQualityString($score, $postType){
		...
		$aReviewQualities = $aReviewSettings['review_qualities'][$mode];
	...
	public static function getAverageReviewsItem($reviewID){
		global $wpdb;
		$postTbl = $wpdb->posts;
		$reviewMetaTbl = $wpdb->prefix . AlterTableReviewMeta::$tblName;
		...
		$score = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT AVG($reviewMetaTbl.meta_value) FROM $reviewMetaTbl LEFT JOIN $postTbl ON ($postTbl.ID = $reviewMetaTbl.reviewID) WHERE $postTbl.ID=%d AND $postTbl.post_type=%s AND $postTbl.post_status='publish'",
				$reviewID, 'review'
			)
		);
