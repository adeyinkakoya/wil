...
class PlanRelationshipModel {
	...
	public static function getUsedNonRecurringPlan(RemainingItems $that){
		...
		return $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT($planRelationshipTbl.objectID) FROM $planRelationshipTbl LEFT JOIN $postTbl ON ($postTbl.ID=$planRelationshipTbl.objectID) LEFT JOIN $historyTbl ON ($historyTbl.ID=$planRelationshipTbl.paymentID) WHERE $planRelationshipTbl.planID=%d AND $planRelationshipTbl.paymentID=%d AND $planRelationshipTbl.userID=%d",
	...
	public static function getUsedRecurringPlan(RemainingItems $that){
		...
		return $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT($planRelationshipTbl.objectID) FROM $planRelationshipTbl WHERE $planRelationshipTbl.planID=%d AND $planRelationshipTbl.paymentID=%d AND $planRelationshipTbl.userID=%d AND $planRelationshipTbl.updatedAtGMT >= (%s - INTERVAL %d DAY)",
