...
if ( document.getElementById('wiloke-design-hero-search-form-wrapper') !== null ){
	new Vue({
		el: "#wiloke-design-hero-search-form-wrapper",
		data: {
			oUsedFields: WILOKE_LISTING_TOOLS.aHeroSearchForm.aUsedFields,
			oAllFields: WILOKE_LISTING_TOOLS.aHeroSearchForm.aAllFields,
			oAvailableFields: (WILOKE_LISTING_TOOLS.aHeroSearchForm.aAvailableFields.length) ? WILOKE_LISTING_TOOLS.aHeroSearchForm.aAvailableFields : [],
			savingXHR: null,
			errorMsg: '',
			successMsg: '',
			postType: WILOKE_LISTING_TOOLS.postType,
			ajaxAction: WILOKE_LISTING_TOOLS.aHeroSearchForm.ajaxAction
		},
		methods: {
			resetDefaults(){
				...
				let $form = jQuery('#wiloke-design-hero-search-form');
				...
				jQuery.ajax({
					...
					url: ajaxurl,
					data: {
						action: 'wilcity_reset_to_default_hero_search_form',
