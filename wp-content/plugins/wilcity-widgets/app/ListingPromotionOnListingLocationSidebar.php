...
class ListingPromotionOnListingLocationSidebar extends \WP_Widget {
	public $aDef = array('title'=>'', 'post_type' => 'listing', 'orderby'=>'menu_order', 'order'=>'DESC', 'number_of_posts'=>4, 'taxonomy'=>'ignore', 'style'=>'list');

	public function __construct() {
		add_filter('wilcity/article-class', array($this, 'articleCssClass'), 10, 2);
		parent::__construct( 'wilcity_promote_listings_on_location', WILCITY_WIDGET . ' (Promotion) Location');
	}
	...
	public function form( $aInstance ) {
		...
		<div class="widget-group">
			...
			<select class="widefat" name="<?php echo $this->get_field_name('order'); ?>" id="<?php echo $this->get_field_id('order'); ?>">
				<?php foreach (array('ASC'=>'The newest Promotions show first', 'DESC'=>'The Oldest Promotions show first') as $order => $name): ?>
		...
		<div class="widget-group">
			...
			<p><i>Self Term: It means all Listings that belongs to the current Location page will be shown. For example: There are two promotion listings: iPhone and SamSung. Listing iPhone belongs to Apple Tag and Listing SamSung belongs to Android Tag. If you are using Self Term option and you are in Apple tag page, the only iPhone will be shown. Otherwise, the both iPhone, and SamSung will be shown.</i></p>
			<select class="widefat" name="<?php echo $this->get_field_name('taxonomy'); ?>" id="<?php echo $this->get_field_id('taxonomy'); ?>">
				<?php foreach (array('self'=>'Self Term', 'ignore'=>'I do want to use this option') as $option => $name): ?>
	...
	public function widget( $aAtts, $aInstance ) {
		...
		$aArgs = array(
			...
			'post_status'       => 'publish',
			'meta_key'          => 'wilcity_promote_listing_location_sidebar',
