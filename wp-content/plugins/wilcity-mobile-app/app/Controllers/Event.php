...
class Event extends JsonSkeleton {
	use BuildQuery;
	private $aUnnecessaryItems = array('businessStatus', 'oTerm', 'oIcon');

	public function __construct() {
		add_action( 'rest_api_init', function () {
			register_rest_route( WILOKE_PREFIX.'/v2', '/event-detail/(?P<target>\w+)', array(
				'methods' => 'GET',
				'callback' => array($this, 'getEvent'),
			));
		});

		add_action('wilcity/mobile/render_event_on_mobile', array($this, 'buildListingEventData'), 5, 2);
		...
		foreach ($aSettings as $aSetting){
			...
			if ( !empty($content) ){
				$aSections[] = array(
					'text'      => $aSetting['name'],
					'type'      => $aSetting['key'],
					'content'   => strip_tags($this->getListingData($aSetting['key'], $post))
				);
			}
		}
