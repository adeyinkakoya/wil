...
class MessageController extends JsonSkeleton {
	use VerifyToken;
	use ParsePost;
	...
	public function deleteAuthorChatted(){
		...
		$status = MessageModel::deleteAuthorMessages($_POST['chatFriendID']);
		if ( $status ){
			return array(
				'status' => 'error',
	...
	public function getAuthorsChatted(){
		...
		$aRawResults = ThemeMessageController::getMessageAuthors($oToken->userID, $aExcludes, $postsPerPage);
