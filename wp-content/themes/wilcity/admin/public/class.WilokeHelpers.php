...
class WilokeHelpers{
	...
	public static function getVimeoThumbnail($vimeoID){
		$url = 'http://vimeo.com/api/v2/video/'.$vimeoID.'.php';
	...
	public static function getTermIcon($oTerm, $iconWrapper='', $hasLink=true){
		$icon = GetSettings::getTermMeta($oTerm->term_id, 'icon_img');

		if ( !empty($icon) ){
			if ( $hasLink ){
				$termLink = get_term_link($oTerm->term_id);
				return '<a href="'.esc_url($termLink).'"><div class="bg-transparent '.esc_attr($iconWrapper).'"><img src="'.esc_url($icon).'" alt="'.esc_attr($oTerm->name).'"></div><div class="icon-box-1_text__3R39g">'.esc_html($oTerm->name).'</div></a>';
			}else{
				return '<img src="'.esc_url($icon).'" alt="'.esc_attr($oTerm->name).'">';
			}
		}
		...
		if ( $hasLink ){
			$termLink = get_term_link($oTerm->term_id);
			if ( !empty($iconColor) ){
				return '<a href="'.esc_url($termLink).'"><div class="'.esc_attr($iconWrapper).'" style="background-color: '.esc_attr($iconColor).'"><i class="'.esc_attr($icon).'"></i></div><div class="icon-box-1_text__3R39g">'.esc_html($oTerm->name).'</div></a>';
			}else{
				return '<a href="'.esc_url($termLink).'"><div class="'.esc_attr($iconWrapper).'"><i class="'.esc_attr($icon).'"></i></div><div class="icon-box-1_text__3R39g">'.esc_html($oTerm->name).'</div></a>';
			}

		}else{
			if ( !empty($iconColor) ){
				return '<div class="'.esc_attr($iconWrapper).'" style="background-color: '.esc_attr($iconColor).'"><i class="'.esc_attr($icon).'"></i></div>';
			}else{
				return '<div class="'.esc_attr($iconWrapper).'"><i class="'.esc_attr($icon).'"></i></div>';
			}
		}
	}
}