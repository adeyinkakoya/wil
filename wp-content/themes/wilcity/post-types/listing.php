...
$logo = GetSettings::getLogo($post->ID, 'full');
...
if ( have_posts() ): while ( have_posts() ): the_post();
	...
	<div id="wilcity-single-listing-content">
		<div class="wil-content">
									...
									<div class="dropdown_itemsWrap__2fuze" data-toggle-content="dropdown">
										<ul class="list_module__1eis9 list-none list_small__3fRoS list_abs__OP7Og arrow--top-right">
											<?php if ( WilokeUser::isPostAuthor($post) ) : ?>
												<li class="list_item__3YghP"><a class="list_link__2rDA1 text-ellipsis color-primary--hover" href="<?php echo esc_url(apply_filters('wilcity/single-listing/edit-listing', null, $post)); ?>"><span class="list_icon__2YpTp"><i class="la la-edit"></i></span><span class="list_text__35R07"><?php esc_html_e('Edit', 'wilcity'); ?></span></a></li>
											<?php endif; ?>

											<?php if ( ReviewModel::isEnabledReview($post->ID, $post->post_type) && !ReviewModel::isUserReviewed($post->ID) ) : ?>
												<li class="list_item__3YghP">
                                                    <wiloke-review-popup-btn v-on:on-open-review-popup="onOpenReviewPopup" review-id="" btn-name="<?php echo esc_attr__('Write a review', 'wilcity'); ?>" icon="la la-star-o" wrapper-class="list_link__2rDA1 text-ellipsis color-primary--hover"></wiloke-review-popup-btn>
                                                </li>
											<?php endif; ?>
											<?php if ( WilokeUser::isPostAuthor($post) ) : ?>
												<li class="list_item__3YghP">
                                                    <wiloke-switch-tab-btn tab-key="listing-settings" wrapper-class="list_link__2rDA1 text-ellipsis color-primary--hover" v-on:on-switch-tab="switchTab">
                                            ...
                                            <li class="list_item__3YghP">
                                                <wiloke-report-popup-btn wrapper-class="list_link__2rDA1 text-ellipsis color-primary--hover" target-id="<?php echo esc_attr($post->ID); ?>"><template slot="insideBtn"><span class="list_icon__2YpTp"><i class="la la-flag-o"></i></span><span class="list_text__35R07"><?php esc_html_e('Report', 'wilcity'); ?></span></template></wiloke-report-popup-btn>
                                            </li>
                                            <?php endif; ?>
										</ul>
									</div>
			...
			<?php do_action('wilcity/single-listing/wil-content', $post); ?>
		</div>
		<?php
