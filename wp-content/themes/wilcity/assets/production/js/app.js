...
"use strict";
    ...
    */
		...
		var adminBarHeight = document.body.wilHasClass(".admin-bar") ? 32 : 0;
		/**
    * Sticky
    */
			...
			_createClass(wilHasValue, [{
				...
				value: function init() {
					...
					this.el.each(function (index) {
						...
						var inputValue = $('input[type="text"], input[type="email"],input[type="url"], textarea', self);
   ...
   */
    ...
    */
		...
		var newWilSlider = new wilSlider('.js-slider');
		...
		var textareaAutoHeight = function () {
			...
			_createClass(textareaAutoHeight, [{
				...
				value: function init() {
					...
					this.el.each(function (index) {
						var self = $(_this14.el[index]);
						var txtFontsize = parseInt(self.css('font-size')),
							    txtLineheight = parseInt(self.css('line-height')),
							    optDefault = {
							height: txtFontsize * (txtLineheight / txtFontsize)
						},
							    optData = {
							height: self.data('height-default')
						},
							    opts = $.extend(optDefault, _this14.opt, optData);
						self.css('height', opts.height + 'px');
						var borderTop = Number(self.css('border-top-width').replace('px', ''));
						var borderBottom = Number(self.css('border-bottom-width').replace('px', ''));
						var minHeight = 0;
						if ($('.js-chatContent-body').length) {
							var chatContent = $('.js-chatContent-body');
							minHeight = chatContent.outerHeight() - 300 - 55;
						...
						var resize = function resize() {
							self.css('height', opts.height + 'px');
							self.css('height', self.prop('scrollHeight') + borderTop + borderBottom + 'px');

							if ($('.js-chatContent-body').length) {
								var chatContent = $('.js-chatContent-body');
								var startHeight = opts.height + 55;
								var newHeight = self.prop('scrollHeight') + borderTop + borderBottom + 55;
								var chatContentHeight = chatContent.prop('scrollHeight');
								if (window.innerWidth < 768) {
								...
								chatContent.css('height', 'calc(100% - ' + (startHeight + 90) + 'px)');
								chatContent.css({
									'height': 'calc(100% - ' + (newHeight + 90) + 'px)',
									'min-height': minHeight + 'px'
								});
								self.css('overflow-y', newHeight + 90 >= 355 ? 'auto' : 'hidden');
							}
						};
						resize();
		...
		function pricing() {
			...
			wilEach(listWraps, function (listWrap) {
				...
				wilEach(lists, function (list) {
					...
					list.appendChild(list.querySelector("del") === null ? check : close);
				});
			});
		}
		pricing();
	...
	var highLightLava = function () {
			...
			_createClass(highLightLava, [{
				...
				value: function lineStyle(rect, line) {
					...
					line.style.left = rect.left - elRect.left + scrollLeft + 'px';
				}
			}, {
   ...
   */
			...
			_createClass(wilMenuVertical, [{
				...
				value: function create(els) {
					...
					wilEach(subMenus, function (subMenu) {
						_this22.createBackButton(menus, subMenu, function (back, menus) {
							_this22.handleClickBack(back, menus);
							_this22.handleClickLink(back, subMenu, menus);
						});
					});
				...
				value: function menuTranslate(menus, level, height) {
					...
					wilEach(menus, function (menu) {
						menu.wilStyles({
							top: _this23.verticalAlign() + '%',
							transform: 'translate(-' + opts.menuWidth * level + 'px, -' + _this23.verticalAlign() + '%)',
				...
				value: function handleClickLink(back, subMenu, menus) {
					...
					menuHasSubMenu.addEventListener('click', function (event) {
						...
						_this24.level++;
						_this24.menuTranslate(menus, _this24.level, subMenuHeight + 'px');
				...
				value: function handleClickBack(back, menus) {
					...
					back.addEventListener('click', function (event) {
						...
						_this25.menuTranslate(menus, _this25.level, parentHeight + 'px');
				...
				value: function create() {
					...
					this.el.on('navResize', function () {
						var update = setInterval(function () {
							...
							var totalWidthHeaderItem = $('.js-header-login').outerWidth(true) + $('.js-header-logo').outerWidth(true) + $('.js-header-search').outerWidth(true) + 100;
							if (maxWidth === 'auto') {
		...
		function toggelEventMap() {
			$('.wilcity-toggle-event-map').on('click', function(event){
				var lat = $(this).data('lat'), lng = $(this).data('lng'), mapID = $(this).data('id');
