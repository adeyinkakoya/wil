...
?>
    <div class="<?php echo esc_attr($aHighlightBoxes['itemsPerRow']); ?>">
        ...
        <div class="icon-box-2_module__AWd3Y wil-text-center" style="background-color: <?php echo esc_attr($aItem['bgColor']); ?>">
            <a class="temporary-disable" href="<?php echo esc_url($linkTo); ?>" target="<?php echo esc_attr($aItem['linkTargetType']); ?>" <?php if ( isset($aItem['isPopup']) && ($aItem['isPopup'] == 'yes') ) : ?> @click.prevent="onOpenPopup('<?php echo esc_attr($aItem['key']); ?>', '<?php echo esc_attr($post->ID); ?>')" <?php endif; ?>>
